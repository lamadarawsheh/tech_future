...
export const Trending: React.FC = () => {
  ...
  // Translate posts
  const translatedPosts = posts.map(post => getTranslatedPost(post, i18n.language) as BlogPost);
  const heroPost = translatedPosts[0];
  const gridPosts = translatedPosts.slice(1);
        ...
        <div className="relative overflow-hidden rounded-2xl bg-slate-900 border border-slate-800">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 p-8 md:p-12">
            <div className="flex flex-col gap-3 md:w-1/2">
              <h3 className="text-2xl md:text-3xl font-bold text-white font-display">{t('trending.subscribeTitle')}</h3>
              <p className="text-slate-400 text-base">{t('trending.subscribeDesc')}</p>
            </div>
            <div className="flex w-full md:w-1/2 max-w-md">
              {user ? (
                <div className="flex items-center gap-3 p-4 bg-green-900/30 rounded-lg border border-green-800 w-full">
                  ...
                  <div>
                    <p className="text-white font-medium">{t('trending.subscribed')}</p>
