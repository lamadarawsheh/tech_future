...
export const Home: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // Fetch one extra post in case we need to fallback for the Hero section
        const [featured, recent, total, tags, trending] = await Promise.all([
          getFeaturedPost(lang),
          getRecentPosts(POSTS_PER_PAGE + 1, 0, lang),
          getTotalPostsCount(lang),
          getPopularTags(8, lang),
          getTrendingPostsByTime('all', 4, lang),
        ]);
        ...
        if (finalFeatured) {
          // If we have an explicit featured post, filter it out from the recent list
          finalRecent = recent.filter(p => p._id !== finalFeatured?._id).slice(0, POSTS_PER_PAGE);
        } else if (recent.length > 0) {
          // Hero Fallback: Use the most recent as featured if no explicit featured post exists
          finalFeatured = recent[0];
          finalRecent = recent.slice(1, POSTS_PER_PAGE + 1);
        }

        setFeaturedPost(finalFeatured);
        setRecentPosts(finalRecent);
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      ...
      const lang = i18n.language;
      try {
        const trending = await getTrendingPostsByTime(trendingFilter, 4, lang);
    ...
    fetchTrending();
  }, [trendingFilter, i18n.language]);

  const loadMorePosts = async () => {
    setLoadingMore(true);
    const lang = i18n.language;
    try {
      // Offset should account for the featured post plus existing recent posts
      const currentOffset = recentPosts.length + (featuredPost ? 1 : 0);
      const morePosts = await getRecentPosts(POSTS_PER_PAGE, currentOffset, lang);
  ...
  const hasMorePosts = (recentPosts.length + (featuredPost ? 1 : 0)) < totalPosts;
        ...
        <section className="mb-16 rounded-3xl overflow-hidden relative group shadow-2xl shadow-slate-200/50 dark:shadow-none">
          ...
          <div className="absolute inset-0 z-20 flex flex-col justify-end p-6 sm:p-10 lg:p-16">
            <div className="max-w-4xl space-y-6 transform transition-transform duration-500 group-hover:-translate-y-2">
              ...
              <div className="pt-4">
                <Link
                  href={`/article/${getSlug(featuredPost.slug)}`}
            ...
            <div className="flex flex-col gap-5">
              {loadingTrending ? (
                ...
                trendingPosts.map((post: BlogPost, index: number) => (
                  <Link
                    key={post._id}
                    href={`/article/${getSlug(post.slug)}`}
