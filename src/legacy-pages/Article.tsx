  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        ...
        // 1. Fetch post (prioritizes lang but finds slug in any lang)
        ...
        // 2. Proactive redirection if language doesn't match
        const postLang = data.language || 'en';
        if (postLang !== lang) {
          const translations = await getDocumentTranslations(data._id);
          const target = translations.find(t => t.value?.language === lang);
          if (target?.value?.slug?.current) {
            router.push(`/article/${target.value.slug.current}`);
            return;
          }
        }
        ...
        <header className="mb-12">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
            <div className="flex items-center gap-2">
              ...
              {post.readingTime && <span>{post.readingTime} {t('article.minRead')}</span>}
            </div>
            <span>•</span>
            ...
            {post.updatedDate && (
              <>
                ...
                <span className="text-xs">
                  {t('article.updated')} {format(new Date(post.updatedDate), 'MMM d, yyyy')}
                </span>
              </>
            )}
          </div>
          ...
          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-slate-200 dark:border-slate-700">
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
              ...
              <span>{post.viewCount || 0} {t('article.views')}</span>
            </div>
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
              <span className="material-symbols-outlined text-[18px]">favorite</span>
              <span>{post.likeCount || 0} {t('article.likes')}</span>
            </div>
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
              <span className="material-symbols-outlined text-[18px]">chat_bubble</span>
              <span>{post.commentCount || 0} {t('article.comments')}</span>
            </div>
          </div>
        </header>
