      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Header Card */}
            <div className={`bg-gradient-to-br ${isDarkMode ? 'from-indigo-500/20 via-purple-500/10 to-pink-500/20 border-indigo-500/30' : 'from-indigo-50 via-white to-purple-50 border-indigo-100'} rounded-2xl border p-6 relative overflow-hidden`}>
              <div className={`absolute top-0 right-0 w-64 h-64 ${isDarkMode ? 'bg-indigo-500/10' : 'bg-indigo-100/50'} rounded-full blur-3xl`} />

              <div className="relative flex items-center justify-between">
                <div>
                  <p className="text-indigo-400 text-sm font-medium mb-1">Current Rank</p>
                  <p className={`text-5xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'} mb-2`}>Top 5%</p>
                  <div className={`flex items-center gap-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'} text-sm`}>
                ...
                <div className="text-center">
                  ...
                  <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-semibold`}>Coding Constellation</p>
                </div>
              </div>

              <p className={`${isDarkMode ? 'text-slate-300' : 'text-slate-600'} text-sm mt-4`}>
            ...
            {!loading && topThree.length >= 3 && (
              <div className="flex items-end justify-center gap-4 py-8">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-semibold text-sm mb-1`}>{topThree[1]?.displayName || 'User'}</p>
                  <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-500'} text-xs mb-2`}>{topThree[1]?.xp?.toLocaleString() || 0} XP</p>
                  <div className={`w-20 h-20 ${isDarkMode ? 'bg-gradient-to-t from-slate-600 to-slate-500' : 'bg-gradient-to-t from-slate-300 to-slate-200'} rounded-t-lg flex items-center justify-center`}>
                    <span className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-600'}`}>2</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center -mb-4">
                  ...
                  <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-bold mb-0.5`}>{topThree[0]?.displayName || 'User'}</p>
                  <p className="text-amber-400 text-xs font-semibold mb-1">LEGENDARY</p>
                  <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-500'} text-xs mb-2`}>{topThree[0]?.xp?.toLocaleString() || 0} XP</p>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-semibold text-sm mb-1`}>{topThree[2]?.displayName || 'User'}</p>
                  <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-500'} text-xs mb-2`}>{topThree[2]?.xp?.toLocaleString() || 0} XP</p>
                  <div className={`w-20 h-16 ${isDarkMode ? 'bg-gradient-to-t from-orange-700 to-orange-600' : 'bg-gradient-to-t from-orange-300 to-orange-200'} rounded-t-lg flex items-center justify-center`}>
                    <span className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-orange-900'}`}>3</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Tabs */}
            <div className={`flex items-center gap-4 border-b ${isDarkMode ? 'border-slate-800' : 'border-slate-200'} pb-4`}>
            ...
            <div className="space-y-2">
              {loading ? (
                Array.from({ length: 10 }).map((_, i) => (
                  <div key={i} className={`${isDarkMode ? 'bg-slate-900/50' : 'bg-white border border-slate-200'} rounded-xl p-4 animate-pulse`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-8 h-8 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'} rounded-lg`} />
                      <div className="flex-1">
                        <div className={`h-4 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'} rounded w-1/4 mb-2`} />
                        <div className={`h-3 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'} rounded w-1/6`} />
                      </div>
                    </div>
                  </div>
                ))
                ...
                restOfLeaderboard.map((user, index) => {
                  ...
                  return (
                    <div
                      key={user._id}
                      className={`flex items-center gap-4 ${isDarkMode ? 'bg-slate-900/50 hover:bg-slate-800/50 border-slate-800' : 'bg-white hover:bg-slate-50 border-slate-200'} border rounded-xl p-4 transition`}
                    >
                      {/* Rank */}
                      <div className="flex items-center gap-2 w-16">
                        <span className={`text-lg font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>{String(rank).padStart(2, '0')}</span>
                        {rankChange === 'up' && (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-medium`}>{user.displayName || 'Anonymous'}</span>
                      ...
                      {/* Level */}
                      <div className="text-center">
                        <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Lvl</p>
                        <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-bold`}>{user.level || 1}</p>
                      </div>

                      {/* XP */}
                      <div className="text-right w-24">
                        <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-bold`}>{user.xp?.toLocaleString() || 0}</p>
                        <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Points</p>
                      </div>
                    </div>
                  );
                })
          ...
          <div className="space-y-6">
            ...
            {userProfile ? (
              <div className={`${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl p-5`}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-semibold`}>{userProfile.displayName || 'You'}</p>
                    <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-600'} text-sm`}>{userProfile.xp?.toLocaleString() || 0} XP</p>
                  </div>
                </div>

                <div className="mb-4">
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className={`${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Level {userProfile.level || 1}</span>
              ...
              <div className={`${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl p-5 text-center`}>
                <span className="material-symbols-outlined text-4xl text-slate-600 mb-3">person_add</span>
                <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-semibold mb-2`}>Join the Leaderboard</p>
                <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-600'} text-sm mb-4`}>Sign in to track your progress and compete with others</p>
                <Link
                  to="/practice"
            ...
            {topThree.length > 0 && (
              <div className={`${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl p-5`}>
                <h3 className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-semibold mb-4 flex items-center gap-2`}>
                ...
                <div className="space-y-3">
                  {topThree.map((user, i) => (
                    <div key={user._id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} text-sm font-medium`}>{user.displayName || 'Anonymous'}</p>
                        <p className={`${isDarkMode ? 'text-slate-500' : 'text-slate-600'} text-xs`}>{user.xp?.toLocaleString() || 0} XP</p>
                      </div>
                      <span className={`text-xs font-medium ${getTierColor(user.tier)}`}>
            ...
            {/* Season Info */}
            <div className={`bg-gradient-to-br ${isDarkMode ? 'from-purple-500/20 to-pink-500/20 border-purple-500/30' : 'from-purple-50 to-pink-50 border-purple-100'} border rounded-2xl p-5`}>
              <div className="flex items-center gap-2 mb-3">
                <span className="material-symbols-outlined text-purple-400">event</span>
                <span className={`${isDarkMode ? 'text-white' : 'text-slate-900'} font-semibold`}>Season Rewards</span>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className={`${isDarkMode ? 'text-white' : 'text-slate-700'}`}>1,000 XP + Crown Badge</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-400">🥈 2nd Place</span>
                  <span className={`${isDarkMode ? 'text-white' : 'text-slate-700'}`}>750 XP + Silver Badge</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-orange-400">🥉 3rd Place</span>
                  <span className={`${isDarkMode ? 'text-white' : 'text-slate-700'}`}>500 XP + Bronze Badge</span>
                </div>
              </div>
            </div>
          </div>
