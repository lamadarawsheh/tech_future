...
// Date filter options
const dateFilters: { key: SearchDateFilter; labelKey: string }[] = [
  { key: 'all', labelKey: 'search.allTime' },
  { key: '24h', labelKey: 'search.past24h' },
  { key: 'week', labelKey: 'search.pastWeek' },
  { key: 'month', labelKey: 'search.pastMonth' },
  { key: 'year', labelKey: 'search.pastYear' },
];
...
export const Search: React.FC = () => {
  ...
  const buildFilters = useCallback(() => ({
    ...
    lang: i18n.language
  }), [query, sortBy, selectedCategory, dateFilter, selectedTags, i18n.language]);
  ...
  }, [query, sortBy, selectedCategory, dateFilter, selectedTags, buildFilters, i18n.language]);
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      ...
      const morePosts = await searchPosts({ ...filters, lang: i18n.language });
  ...
  }, [query, posts.length, loadingMore, sortBy, selectedCategory, dateFilter, selectedTags, hasMore, totalResults, i18n.language]);
    ...
    <div className="flex flex-col gap-6 mb-8">
      ...
      <div className="flex flex-col gap-6 mb-8">
        <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
          ...
          <span className={`material-symbols-outlined text-[16px] ${i18n.language === 'ar' ? 'rotate-180' : ''}`}>chevron_right</span>
          <span className="text-slate-900 dark:text-white">{t('search.title')}</span>
        </div>
        <div className="w-full max-w-3xl">
          <label className="flex flex-col h-14 w-full relative">
            <div className="flex w-full flex-1 items-stretch rounded-xl h-full shadow-lg shadow-black/5 dark:shadow-black/20">
              <div className="text-slate-400 dark:text-slate-500 flex border-none bg-white dark:bg-slate-800 items-center justify-center pl-4 rounded-l-xl rtl:pl-0 rtl:pr-4 rtl:rounded-l-none rtl:rounded-r-xl">
                <span className="material-symbols-outlined text-[24px]">search</span>
              </div>
              <input
                className="flex w-full min-w-0 flex-1 bg-white dark:bg-slate-800 text-slate-900 dark:text-white border-none focus:ring-0 h-full placeholder:text-slate-400 px-4 text-lg font-medium rtl:text-right"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder={t('search.placeholder')}
              />
              <div className="flex items-center justify-center rounded-r-xl bg-white dark:bg-slate-800 pr-2 rtl:pr-0 rtl:pl-2 rtl:rounded-r-none rtl:rounded-l-xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
          <div>
            <h1 className="text-slate-900 dark:text-white text-3xl font-bold leading-tight mb-2 font-display">
              {query ? t('search.resultsFor', { query }) : t('search.browse')}
            </h1>
            <p className="text-slate-500 dark:text-slate-400">
              {loading ? t('nav.searching') : t('search.found', { count: totalResults })}
              {selectedCategory !== 'all' && ` ${t('search.in')} ${categories.find(c => getSlug(c.slug) === selectedCategory)?.title || selectedCategory}`}
              {selectedTags.length > 0 && ` ${t('search.taggedWith')} ${selectedTags.join(', ')}`}
              {dateFilter !== 'all' && ` ${t('search.from')} ${t(dateFilters.find(d => d.key === dateFilter)?.labelKey || '').toLowerCase()}`}
            </p>
          </div>
          <div className="flex gap-3 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            {sortOptions.map(({ key, label }) => (
              <button
                ...
                className={`flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-lg px-4 font-medium text-sm transition-colors ${sortBy === key
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <aside className="lg:col-span-3 space-y-8">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {dateFilter !== 'all' && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs rounded-lg">
                    ...
                    {t(dateFilters.find(d => d.key === dateFilter)?.labelKey || '')}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <label className="group flex items-center gap-3 rounded-lg p-2 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-colors -mx-2">
                ...
                <span className="text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white text-sm font-medium flex-1">{t('search.allCategories')}</span>
          ...
          <div className="flex flex-col gap-4">
            <h1 className="text-slate-900 dark:text-white font-bold text-base flex items-center gap-2 font-display">
              ...
              {dateFilters.map((filter) => (
                <label key={filter.key} className="group flex items-center gap-3 p-1 cursor-pointer">
                  <input
                    ...
                    name="dateFilter"
                  />
                  <span className="text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white text-sm">{t(filter.labelKey)}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <h4 className="text-slate-900 dark:text-white font-bold text-base flex items-center gap-2 font-display">
            ...
            <div className="flex flex-wrap gap-2">
              {tags.length > 0 ? (
                tags.map((tagItem) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-md text-xs transition-colors border ${isSelected
                ...
                <p className="text-sm text-slate-500">{t('search.noTags')}</p>
              )}
            </div>
          </div>
        </aside>
        ...
        <div className="lg:col-span-9 flex flex-col gap-6">
                  ...
                  <div className="flex flex-col flex-1 gap-2">
                    <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span>{post.readingTime} {t('home.minRead')}</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto pt-3 flex items-center gap-4">
                      ...
                      <Link to={`/article/${getSlug(post.slug)}`} className="text-primary text-sm font-semibold flex items-center gap-1 group/btn hover:underline">
                        ...
                        <span className={`material-symbols-outlined text-[16px] group-hover/btn:translate-x-1 transition-transform ${i18n.language === 'ar' ? 'rotate-180' : ''}`}>arrow_forward</span>
                      </Link>
                    </div>
                  </div>
                </article>
          ...
          {!loading && posts.length > 0 && (
            <div className="flex flex-col items-center justify-center pt-8 gap-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {t('search.showing', { count: posts.length, total: totalResults })}
              </p>
              {hasMore && (
                <>
          ...
          {!loading && posts.length === 0 && (
            <div className="py-16 text-center">
              ...
              <h3 className="text-xl font-bold text-slate-700 dark:text-slate-300 mb-2">{t('search.noResults')}</h3>
