...
import { Category, getSlug } from '../types';
    ...
    <div className="w-full">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary/10 via-secondary/10 to-primary/5 dark:from-primary/20 dark:via-secondary/20 dark:to-primary/10 p-8 md:p-12 mb-12 border border-primary/20 dark:border-primary/30">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-slate-200 dark:border-slate-700 mb-4">
            ...
            <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{t('nav.categoriesCount', { count: categories.length })}</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((category, index) => {
            const iconMap: Record<string, string> = {
              ...
              'mobile': 'smartphone',
              // Support for Arabic titles if slugs match (unlikely but safe)
              'ذكاء-اصطناعي': 'smart_toy',
              'تطوير': 'code',
            };
            ...
            const icon = iconMap[getSlug(category.slug)] || iconMap[category.title.toLowerCase()] || 'folder';
                ...
                <div className="relative z-10">
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs font-medium text-slate-600 dark:text-slate-400">
                      ...
                      <span>{Math.floor(Math.random() * 50) + 10} {t('categories.articles')}</span>
                    </div>
      ...
      {!loading && categories.length === 0 && (
        <div className="text-center py-12">
          <p className="text-slate-500 dark:text-slate-400">{t('categories.noCategories', 'No categories found.')}</p>
        </div>
      )}
