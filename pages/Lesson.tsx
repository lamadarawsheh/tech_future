  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const lessonData = await client.fetch(query, { slug });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
            lesson.type === 'concept' ? 'bg-blue-500/20 text-blue-400' :
            lesson.type === 'exercise' ? 'bg-green-500/20 text-green-400' :
            ...
            {lesson.type === 'quiz' && lesson.quiz && lesson.quiz.length > 0 && (
              <div className="space-y-6 mt-8">
                ...
                {lesson.quiz.map((question, qIndex) => (
                  <div key={qIndex} className="bg-slate-800/50 rounded-xl p-6">
                ...
                {!quizSubmitted && (
                  <button
                    ...
                    className="w-full py-3 bg-purple-500 hover:bg-purple-600 disabled:bg-slate-700 disabled:cursor-not-allowed text-white rounded-xl font-medium transition"
                  >
                    Submit Answers
                  </button>
                )}
            ...
            {lesson.type === 'challenge' && (
              <div className="mt-8 bg-gradient-to-br from-amber-500/20 to-orange-500/20 border border-amber-500/30 rounded-xl p-6">
                <div className="flex items-center gap-3 mb-4">
                  <span className="material-symbols-outlined text-amber-400 text-2xl">swords</span>
                  <div>
                    <h3 className="text-xl font-bold text-white">Boss Challenge</h3>
                    <p className="text-slate-400 text-sm">Complete this challenge to finish the lesson</p>
                  </div>
                </div>
                <Link
          ...
          {!isCompleted && lesson.type !== 'quiz' && (
            <div className="p-6 border-t border-slate-800 bg-slate-900/30">
              <button
                ...
                disabled={!subscriber}
                className="w-full py-3 bg-cyan-500 hover:bg-cyan-600 disabled:bg-slate-700 disabled:cursor-not-allowed text-white rounded-xl font-medium transition flex items-center justify-center gap-2"
              >
                <span className="material-symbols-outlined">check_circle</span>
